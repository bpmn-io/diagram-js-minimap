...
export default function Minimap(
  ...
  function setViewboxCenteredAroundClickEvent(event) {
    const { eventClientX, eventClientY } = getPositionFromEvent(event);
    ...
    var diagramPoint = mapMousePositionToDiagramPoint({
      x: eventClientX - self._state._svgClientRect.left,
      y: eventClientY - self._state._svgClientRect.top
  ...
  domEvent.bind(this._svg, eventNameAdapter('click'), function(event) {
  ...
  function mousedown(center) {
    ...
    return function onMousedown(event) {
      ...
      var offsetViewportDom = {
        x: eventClientX - initialViewportDomRect.left + 1,
        y: eventClientY - initialViewportDomRect.top + 1
      };
      ...
      domEvent.bind(document, eventNameAdapter('mousemove'), onMousemove);
      domEvent.bind(document, eventNameAdapter('mouseup'), onMouseup);
    };
  }
  ...
  function onMousemove(event) {
    ...
    if (self._state.isDragging) {
      ...
      assign(self._viewportDom.style, {
        top: (eventClientY - offsetViewportDom.y - parentClientRect.top) + 'px',
        left: (eventClientX - offsetViewportDom.x - parentClientRect.left) + 'px'
      });

      // update overlay
      var clipPath = getOverlayClipPath(parentClientRect, {
        top: eventClientY - offsetViewportDom.y - parentClientRect.top,
        left: eventClientX - offsetViewportDom.x - parentClientRect.left,
  ...
  function onMouseup(event) {
    ...
    if (self._state.isDragging) {
      ...
      if (self._state.initialDragPosition.x === eventClientX
        && self._state.initialDragPosition.y === eventClientY) {
        setViewboxCenteredAroundClickEvent(event);
      }
      ...
      domEvent.unbind(document, eventNameAdapter('mousemove'), onMousemove);
      domEvent.unbind(document, eventNameAdapter('mouseup'), onMouseup);
    }
  }

  // dragging viewport scrolls canvas
  domEvent.bind(this._viewportDom, eventNameAdapter('mousedown'), mousedown(false));
  domEvent.bind(this._svg, eventNameAdapter('mousedown'), mousedown(true));
  ...
  domEvent.bind(this._toggle, eventNameAdapter('click'), function(event) {
    event.preventDefault();
    event.stopPropagation();
...
Minimap.prototype._init = function() {
  ...
  domEvent.bind(parent, eventNameAdapter('mousedown'), function(event) {
    event.stopPropagation();
  });
...
function eventNameAdapter(eventName) {
  if (isMobile()) {
    const map = {
...
function getPositionFromEvent(event) {
  if (event.touches) {
    ...
    return {
      eventClientX: firstTouchEvent.clientX,
      eventClientY: firstTouchEvent.clientY,
    };
  }
  return {
