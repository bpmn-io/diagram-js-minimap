...
// default delay (in ms) used to batch (debounce) minimap updates
var DEFAULT_DEBOUNCE_DELAY = 100;

// maximum time (in ms) updates may be debounced before a refresh is forced,
// ensuring a semi-live view during very long running operations (e.g. import)
var DEFAULT_DEBOUNCE_SKIP_DELAY = 2000;
...
export default function Minimap(
  ...
  // whether the element tree changed while the minimap was closed and
  // therefore needs to be rebuilt on the next open
  this._dirty = false;
  ...
  eventBus.on([ 'shape.added', 'connection.added' ], function(context) {
    ...
    // skip element mutations while closed; rebuild on open instead
    if (!self.isOpen()) {
      self._dirty = true;
    ...
    self._addElement(context.element);
  ...
  eventBus.on([ 'shape.removed', 'connection.removed' ], function(context) {
    ...
    self._removeElement(context.element);
  ...
  eventBus.on([ 'root.set', 'plane.set' ], function(event) {
    ...
    // skip rebuild while closed; rebuild from active root on open instead
    if (!self.isOpen()) {
      self._dirty = true;
  ...
  // clean up pending updates on destroy
...
Minimap.prototype._init = function() {
  ...
  domClasses(parent).add('djs-minimap').add('bio-theme-parent');
 ...
 *
 * Batches rapid element changes (e.g. during import or copy-paste) into a
 * single update. If updates keep coming, a refresh is forced after
 * `debounceSkipDelay` ms so a semi-live view is kept during long operations.
 */
Minimap.prototype._scheduleUpdate = function() {
  var self = this;

  // never update while closed
  if (!this.isOpen()) {
    return;
  }

  var debounceDelay = isNumber(this._config.debounceDelay)
    ? this._config.debounceDelay
    : DEFAULT_DEBOUNCE_DELAY;

  var debounceSkipDelay = isNumber(this._config.debounceSkipDelay)
    ? this._config.debounceSkipDelay
    : DEFAULT_DEBOUNCE_SKIP_DELAY;

  var now = Date.now();

  // force an update if we have been debouncing for too long already
  if (this._firstDebounceTime && now - this._firstDebounceTime >= debounceSkipDelay) {
    this._update();
  ...
  this._firstDebounceTime = this._firstDebounceTime || now;
...
/**
 * Rebuild the minimap element tree from the currently active root.
 */
Minimap.prototype._rebuild = function() {
  this._clear();

  var canvas = this._canvas;

  var root = canvas.getRootElement && canvas.getRootElement();
...
Minimap.prototype.open = function() {
  ...
  // rebuild if the element tree changed while we were closed
  if (this._dirty) {
    this._rebuild();
...
Minimap.prototype._addElement = function(element) {
  ...
  // don't render hidden elements (e.g. children of a collapsed shape);
  // this mirrors the `display: none` diagram-js applies to hidden elements
  if (element.hidden) {
    return;
  }
  ...
  if (newElementGfx) {
    ...
    // determine child order via business object relationships instead of
    // expensive DOM queries (see #96)
    var index = getIndexOfChildInParent(element);

    // index can be 0
    if (index !== -1) {
      if (newElementParentGfx) {

        // insert at index; falls back to appending as last child when the
...
/**
 * Returns the index of an element within its parent's children using the
 * business object (diagram) relationship.
 *
 * This avoids the expensive DOM traversal that dominated minimap rendering for
 * large diagrams (see #96).
 *
 * @param {Object} element
 *
 * @return {number} index of the element, or -1 if it cannot be determined
 */
...
/**
 * Insert a minimap element clone at the given index among the parent's other
 * element clones.
 *
 * Element clones are appended as direct children of `parentGfx` (alongside the
 * cloned visual content), so we filter for the id-carrying clones to determine
 * ordering. Falls back to appending when the index is out of range.
 */
function insertChildAtIndex(childGfx, parentGfx, index) {
  var children = getMinimapChildElements(parentGfx);

  var childAtIndex = children[index];

  if (childAtIndex) {
    parentGfx.insertBefore(childGfx, childAtIndex);
  } else {
...
/**
 * Returns the minimap element clones nested in the given parent clone.
 *
 * Only element clones carry an id (visual content is sanitized), so the id acts
 * as the discriminator.
 */
function getMinimapChildElements(parentGfx) {
  return Array.from(parentGfx.childNodes).filter(function(node) {
    return node.id;
  });
}
