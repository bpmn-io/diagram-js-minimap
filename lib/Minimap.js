...
function Minimap(canvas, elementRegistry, eventBus) {
  ...
  eventBus.on('element.updateId', function(ctx) {
    ...
    self.updateElementId(element, newId);
  });
...
Minimap.prototype.toggle = function(open) {
  ...
  this.onToggleCallbacks.forEach(function(cb) {
    cb({
      isOpen: self._state.isOpen
    });
  })
};

Minimap.prototype.onToggle = function(callback) {
  this.onToggleCallbacks.push(callback);
};

Minimap.prototype.offToggle = function(callback) {
  this.onToggleCallbacks = this.onToggleCallbacks.filter(function(cb) {
    return cb !== callback;
  });
};

Minimap.prototype.updateElement = function(element) {

  // if parent is null element has been removed
...
Minimap.prototype.updateElementId = function(element, newId) {
