...
// Default debounce delay for batching updates (in milliseconds)
...
export default function Minimap(
  ...
  // Debounce delay for batching updates (can be configured)
  this._debounceDelay = (config && config.debounceDelay) || DEFAULT_DEBOUNCE_DELAY;

  // Track pending update timer
  this._updateTimer = null;

  // Cache for element index lookups to avoid expensive DOM queries
  ...
  // Use debounced update to batch multiple additions (e.g., during import)
  ...
  // Use debounced update to batch multiple removals
  ...
  // Use debounced update for batch changes
  ...
  // Use immediate update for responsive viewport changes
  ...
  // Handle root/plane changes with immediate update after batch addition
  eventBus.on([ 'root.set', 'plane.set' ], function(event) {
    ...
    // Use immediate update here since this is typically end of a complete operation
    self._updateImmediate();
  });
 ...
 * This batches multiple element changes into a single update for performance.
 */
...
/**
 * Perform an immediate update, canceling any pending debounced updates.
 * Use this for critical operations like viewport changes or user interactions.
 */
Minimap.prototype._updateImmediate = function() {
...
Minimap.prototype._addElement = function(element) {
  ...
  if (newElementGfx) {
    ...
    // Use business object relationships to find index instead of DOM queries
...
/**
 * Returns index of element in parent's children using business object relationships.
 * This is much faster than DOM queries.
 *
 * @param {Object} element - The element to find the index of
 * @return {number} The index of the element in its parent's children, or -1 if not found
 */
